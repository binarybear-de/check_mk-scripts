...
if [ $CLIENTS -lt $WARN_CLIENTS ]; then
	STATE=0
elif [ $CLIENTS -ge $WARN_CLIENTS ] && [ $CLIENTS -lt CRIT_CLIENTS ]; then
	STATE=1
elif [ $CLIENTS -ge $CRIT_CLIENTS ]; then
	STATE=2
else
	STATE=3
fi
...
if [ $(uci get wireless.radio0.disabled) == 1 ] ; then
	...
	DESCRIPTION="$(uci get wireless.radio0.txpower) dBm, $CLIENTS Clients: $CLIENTS"
fi
